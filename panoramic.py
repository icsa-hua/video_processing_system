...
def panorama_to_plane(panorama_path, FOV, output_size, yaw, pitch):
    ...
    y = H / 2 - v
    z = f
#x, y, z, W, H, f, yaw_radian, pitch_radian
    ...
    # source_img = np.array(panorama)
    # color = (int(source_img[V][U][0]), int(source_img[V][U][1]), int(source_img[V][U][2]))
    # output_image
...
for deg in tqdm(np.arange(0, 360, 0.25)):
    output_image = panorama_to_plane('Street View 360.jpg', 90, (600, 600), deg, 90)
