from ..interface.streaming import YOLOStreamer
...
class Yolov5Streamer(YOLOStreamer): 
            ...
            for self.batch in self.dataset: 
                ...
                with profilers[1]: 
                    if self.seen == 0: 
                        ...
                        prof.export_chrome_trace("trace_yolov5.json")
                    else: 
                        preds = self.inference(images, *args, **kwargs)
    ...
    def data_to_tensor_filter(self, boxes, scores, class_ids): 
        ...
        result_boxes = self.non_max_suppression(opt=None, detections=boxes,scores=scores, conf=0.4, iou=0.25, thr=0.5)
