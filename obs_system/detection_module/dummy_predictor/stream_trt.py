...
class TensorRTRTXStreamer(OptimizedStreamer): 
    ...
    def stream_inference(self, source, model, producer_flag, queue, *args, **kwargs): 
        ...
        #     if tile_flag : 
        #         logger.info("Run Inference with Tiles")
        ...
        #         )
        #
        #     logger.info("Run Inference without Tiles")
    ...
    def _stream_inference_impl(self, **kwargs): 
        ...
        for self.batch in self.dataset: 
            ...
            for boxes, scores, cls_, orig_img in zip(i_boxes, i_scores, i_classes, im0s): 
                ...
                if self.seen == len(self.batch)-1 and self.args.verbose: 
                    elapsed_time=time.perf_counter() - start_time 
                    logger.info(f"Time from capturing batch to meaningful information: {elapsed_time:.2f}")
