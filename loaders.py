...
def cropping(im, crop_top, crop_bot, crop_left,crop_right):
    """
    Crops an image to a specified width and height.
    """
    height, width = im.shape[:2]
    crop_top = min(crop_top, height)
    crop_bot = min(crop_bot, height)
    crop_left = min(crop_left, width)
    crop_right = min(crop_right, width)

    cropped_frame = im[crop_top:height - crop_bot, crop_left:width - crop_right]

    return cropped_frame


def zooming_in(im, zoom_factor):
    """
    Zooms in on an image by a specified factor.
    """
    height, width = im.shape[:2]
    ...
    # Get the top-left corner of the cropped area
    start_x = int(center_x - (crop_width // 2))
    start_y = int(center_y - (crop_height // 2))
    crop_frame = im[start_y:start_y + crop_height, start_x:start_x + crop_width]
    
    return cv2.resize(crop_frame, (width, height), interpolation=cv2.INTER_LINEAR)
...
class LoadStreams:
    ...
    def __init__(self, sources="file.streams", vid_stride=1, buffer=False):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            im = cropping(im, 100, 0, 25, 25)
            im = zooming_in(im,1.50)
